...
const acceptGarageRequest = async (garageId, requestId, note) => {
  ...
  if (!["SEARCHING_GARAGE", "GARAGE_ASSIGNED"].includes(request.booking.status)) {
  ...
  const updatedRequest = await prisma.$transaction(async (tx) => {
    ...
    if (!["SEARCHING_GARAGE", "GARAGE_ASSIGNED"].includes(freshBooking.status)) {
    ...
    if (request.booking.requestType === "SOS") {
      ...
      if (!wallet || wallet.balance < 50) {
        throw new ApiError(400, "Customer has insufficient wallet balance for SOS");
      }

      const balanceAfter = wallet.balance - 50;
    ...
    if (request.garage.wallet && request.garage.wallet.balance >= GARAGE_ACCEPT_FEE) {
